...
def solve_laplace(domain, facet_markers, n_dbc, visualize=False):
    """
    ...
    J = fem.Function(W)
    J_expr = fem.Expression(
        as_vector((-uh.dx(0), -uh.dx(1))), W.element.interpolation_points()
    )
    J.interpolate(J_expr)
    J_mag = fem.Function(U)
    J_mag_expr = fem.Expression(
        (uh.dx(0) ** 2 + uh.dx(1) ** 2) ** 0.5, U.element.interpolation_points()
    )
    ...
    if visualize:
        ...
        midpoints = mesh.compute_midpoints(
            domain, domain.topology.dim, range(num_cells)
        )

        num_dofs = W.dofmap.index_map.size_local + W.dofmap.index_map.num_ghosts
        assert num_cells == num_dofs
        values = np.zeros((num_dofs, 3), dtype=np.float64)
        values[:, : domain.geometry.dim] = J.x.array.real.reshape(
            num_dofs, W.dofmap.index_map_bs
        )
        plotter = pyvista.Plotter()
